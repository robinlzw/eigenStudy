...
public:
	...
	void Build(UDVmat & pca, const VSConstBuffer<unsigned> & mapPos, const VSConstBuffer<VSConstBuffer<VFVECTOR3>> & framePos, const float & ratio)
	{
		MemberInit();
		DynamicCompress(mapPos, framePos, ratio);
...
private:
	void      MemberInit();
	void      DynamicCompress(const VSConstBuffer<unsigned> & mapPos, const VSConstBuffer<VSConstBuffer<VFVECTOR3>> & framePos, float ratio);	
	void      MeanVector(const VSConstBuffer<VSConstBuffer<VFVECTOR3>> & framePos);
	void      CreatFrameMat(const VSConstBuffer<unsigned> & mapPos, const VSConstBuffer<VSConstBuffer<VFVECTOR3>> & framePos);
	void      SVDProcess();
