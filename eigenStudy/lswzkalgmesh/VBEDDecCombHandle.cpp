...
void VBEDDecCombHandle::MemberInit()
{
	Rmax = 1 << 31;
	Rmin = 1 << 23;
...
void VBEDDecCombHandle::CombFunc(const VNWZKALG::VSEdgeBreakerDecInfo & dec, const float & deltaCoeff)
{
	//clers decompress
	VSConstBuffer<char>   decClers;
	m_objCLERSDec.Build(decClers, dec.TopoBin, dec.triLen - 1);
	...
	MergeToDouble(dec, deltaCoeffD);

	if (dec.bndryLen > 0)
	{
		m_vDeltaTemp.resize(m_vDelta.size());
		memcpy(&m_vDeltaTemp.front().x, &m_vDelta.front().x, sizeof(VFVECTOR3) * m_vDelta.size());

		//double
		m_vDeltaTempD.resize(m_vDeltaD.size());														 ////////
		memcpy(&m_vDeltaTempD.front().x, &m_vDeltaD.front().x, sizeof(DLE3) * m_vDeltaD.size());	 ////////
	}
	else
	{
		...
		//double
		m_vDeltaTempD.resize(m_vDeltaD.size() + 1);
	...
	HandleCmpRes  clers;
	clers.cmpRes = decClers;
	//clers.firstVert = dec.firstVert;
	clers.delta = VD_V2CB(m_vDeltaTemp);    //////////////
	clers.deltaD = VD_V2CB(m_vDeltaTempD);   //double
	clers.handles = dec.handles;
	clers.bndryLen = dec.bndryLen;
	clers.bndryVt = dec.bndryV;
	clers.delVertID = dec.delVertID;    //ÒªÉ¾³ýµÄ¶¥µã
	m_objDec.Build(msh, clers);
}

void VBEDDecCombHandle::MergeToFloat(const VNWZKALG::VSEdgeBreakerDecInfo & dec, const float & deltaCoeff)
{
...
void VBEDDecCombHandle::Decode(const VNWZKALG::VSEdgeBreakerDecInfo & dec)
{
	TypeTran(dec);
	...
	if (dec.bndryLen > 0)
	{
		objDecX.Build(xHighBit, xh, 1 << dec.compRes.pData[0].bitLen.x, dec.vertLen);		 /////////////
		objDecY.Build(yHighBit, yh, 1 << dec.compRes.pData[1].bitLen.x, dec.vertLen);		 /////////////
		objDecZ.Build(zHighBit, zh, 1 << dec.compRes.pData[2].bitLen.x, dec.vertLen);		 /////////////
	}
	else
	{
...
void VBEDDecCombHandle::TypeTran(const VNWZKALG::VSEdgeBreakerDecInfo & dec)
{
	...
	if (dec.bndryLen > 0)
	{
		LowBitPorcess(m_vLowBitValX, dec.compRes.pData[0].lowBitVal, (dec.vertLen) * dec.compRes.pData[0].bitLen.y, dec.compRes.pData[0].bitLen.y);	 /////////////
		LowBitPorcess(m_vLowBitValY, dec.compRes.pData[1].lowBitVal, (dec.vertLen) * dec.compRes.pData[1].bitLen.y, dec.compRes.pData[1].bitLen.y);	 /////////////
		LowBitPorcess(m_vLowBitValZ, dec.compRes.pData[2].lowBitVal, (dec.vertLen) * dec.compRes.pData[2].bitLen.y, dec.compRes.pData[2].bitLen.y);	 /////////////
	}
	else
	{
...
void VBEDDecCombHandle::LowBitPorcess(std::vector<unsigned>& lowBitValue, const VSConstBuffer<char>& lowBitChar, const unsigned & totalLowBitLen, const unsigned & lowBitLen)
{
...
void VBEDDecCombHandle::MergeToDouble(const VNWZKALG::VSEdgeBreakerDecInfo & dec, const double deltaCoeff)
{
	VSConstBuffer<unsigned>   highX = VD_V2CB(m_vHighBitValX);
	VSConstBuffer<unsigned>   highY = VD_V2CB(m_vHighBitValY);
	VSConstBuffer<unsigned>   highZ = VD_V2CB(m_vHighBitValZ);
...
void VBEDDecCombHandle::MergeD(std::vector<double>& merg, const VSConstBuffer<unsigned>& highBit, const VSConstBuffer<unsigned>& lowBit, const double delta, const unsigned & lowBitLen, const int & min)
{
