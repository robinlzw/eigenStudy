...
void test1()
{
	...
	auto generateMeshWrong = [](VSSimpleMeshF& mesh)		
	{
	...
	auto generateMeshRight = [](VSSimpleMeshF& mesh)		
	{
...
// 测试VSMesh类——行为类似值的网格类型
void test8() 
{
	NM_PMMESH::VSMesh vmesh;			// 注意有命名空间。
	VSSimpleMeshF smesh;

	OBJReadSimpMesh(smesh, "E:/tooth.obj");		// 数据在堆上，数组形式连续存储。

	// 1. 拷贝simplemesh网格指针指向的数据，输出vsmesh对象——貌似必须使用数据拷贝，没有办法将原数据封装为vsmesh格式。
	PMCopyMesh(vmesh, smesh);

	// 2. vsmesh对象的拷贝
	NM_PMMESH::VSMesh vmesh2;
	PMCopyMesh(vmesh2, vmesh);

	// 3. 得到vsmesh网格的simplemesh格式的网格指针————不发生数据拷贝。
	PMGetSimpMesh(smesh, vmesh);
...
void test9() 
{
	...
	// simplemesh指向的网格转换为perfectmesh格式的网格。
	GETPERFECTMESH(pmesh, smesh, extSys, rb);		


	// 比较一下三种网格类型对象大小：？？？不知道如何解释这个结果。
	cout << sizeof(smesh) << endl;
	cout << smesh.nTriangleCount * sizeof(smesh.pTriangles[0]) + smesh.nVertCount * sizeof(smesh.pVertices[0]) << endl;
	cout << sizeof(vmesh) << endl;
	cout << sizeof(vmesh.vVertice) + sizeof(vmesh.vSurface) << endl;
	cout << sizeof(pmesh) << endl;
...
// 研究一下VSESS<>, TVR2B<>, TVExtSource<>等几个经常用到但是定义太复杂的类模板。
void test10() 
{
	...
	VSPerfectMesh pmesh;
	OBJReadSimpMesh(smesh, "E:/tooth.obj");
	typedef VNALGMESH::VRPerfMesh VRPerfMesh;					// 貌似是关系类


	//		GETPERFECTMESH(VSPerfectMesh对象,VSSimpleMeshF对象, suf1, suf2)， 两个后缀名suf1, suf2可以随便写，只是中间变量的后缀名。
	TVExtSource<VRPerfMesh> perf_suf1;
